...
class ColumnDefinition
{
    ...
    /**
     * CSS-класс, который долженн быть назначен каждой ячейке (<td>) столбца таблицы.
     * Если нужно назначить несколько классов, то их следует разделить пробелом. Значение по умолчанию: ''.
     *
     * @var string
     */
    public $classes = '';
    ...
    /**
     * Тип форматирования значений ячеек в данном столбце.
     * Допустимые значения: integer, number, currency, date, email, link, showlink, checkbox, select и actions. Значение по умолчанию: ''.
     *
     * @var string
     */
    public $formatter = '';
    ...
    /**
     * Массив параметров форматирования. Для формирования значения можно воспользоваться
     * помощниками {@see \Rusproj\FreeJqGridConfigurator\JqGrid\ColumnDefinition::format}.
     * Значение по умолчанию: [].
     *
     * @var array
     */
    ...
    /**
     * Массив, содержащий параметры поиска. Для формирования значения можно воспользоваться
     * помощником {@see \Rusproj\FreeJqGridConfigurator\JqGrid\ColumnDefinition::searchOptions}.
     * Значение по умолчанию: [].
     *
     * @var array
     */
    ...
    public function getConfig() {
        ...
        if ($this->search) {
            ...
            if (count($this->searchOptions) > 0) {
                ...
                $cfg['searchoptions'] = ColumnDefinition::searchOptions();
            }
        }
        if ($this->fixed) {
            $cfg['fixed'] = true;
    ...
    /**
     * Помощник, представляющий возможность сформировать значение дополнительных параметров formatOptions для типа редактирования formatter = 'select'.
     * Для отображения текста использовать те же значения, которые определены в edittype = 'select'.
     *
     * @return array Массив дополнительных параметров, который может применяться для задания значения formatOptions.
     */
