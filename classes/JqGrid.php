<?php
namespace RUSproj\Kohana\JqGrid;
...
class JqGrid
{
    ...
    /**
     * Задает массив описаний всех выводимых полей. Для задания параметров отдельных столбцов удобно пользваться статическими методами класса ColumnConfig или задав необходимые свойства экземпляру класса. Значение по умолчанию: [] (не задано).
     * @var JqGrid_Column[]
     */
    public $colModel = [];

    /**
     * Задает массив названий всех выводимых полей. Число записей в данном массиве должно соответсвовать числу записей в массиве свойства colModel. В качестве альтернативы, названия столбцов можно задавать в конфигурационном массиве colModel. Данное свойство имеет силу, если задано свойство colModel. Значение по умолчанию: [] (не задано).
     * @var array
     */
    public $colNames = [];
    ...
    /**
     * Массив дополнительных параметров субтаблиц. Данный параметр имеет место, если значение свойства sortOrder установлено в true. Значение по умолчанию: [] (не задано).
     * @var array
     */
    public $subGridOptions = [];

    /**
     * Массив, описывающий модель вложенных субтаблиц. Для задания параметров отдельных столбцов удобно пользваться статическими методами класса ColumnConfig или задав необходимые свойства экземпляру класса и вызвав метод getConfig(). Данный параметр имеет место, если значение свойства sortOrder установлено в true. Значение по умолчанию: [] (не задано).
     * @var array
     */
    public $subGridModel = [];
    ...
    /**
     * Задает расположение панели инструментов. Первый параметр массива определяет признак необходимости включить панель инструментов. Второй определяет место создания панели. Возможные значения: top (наверху, перед таблицей), bottom (внизу, после таблицы), both (вверху и внизу). Значение по умолчанию: [false, 'both'].
     * @var array
     */
    ...
    /**
     * Задает значки, используемые для представления дерева. Необходима следующая структура массива: {plus:'ui-icon-triangle-1-e',minus:'ui-icon-triangle-1-s',leaf:'ui-icon-radio-off'}. Данный параметр имеет место, если значение свойства treeGred установлено в true. Значение по умолчанию: [] (не определено).
     * @var array
     */
    public $treeIcons = [];

    /**
     * Расширяет свойство colModel в базовой таблице. Описанные здесь поля добавляются в конец свойства colModel и скрываются. Это означает, что данные, возвращаемые с сервера должны иметь значения для этих полей. Данный параметр имеет место, если значение свойства treeGred установлено в true. Значение по умолчанию: [] (не определено).
     * @var array
     */
    public $treeReader = [];
